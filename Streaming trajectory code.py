...
STREAM_FILE_NAME = "mechanical_stage_lissajous.csv"
        ...
        try:
            ...
            if event.type == ctl.EventType.STREAM_FINISHED:
                ...
                if ctl.EventParameter.PARAM_RESULT(event.i32) == ctl.ErrorCode.NONE:
                    ...
                    # Note: The event parameter now holds the error code as well as the 
                    # channel index responsible for the failure
                    print("MCS2 stream aborted by device: {}".format(ctl.ErrorCode(
                    ctl.EventParameter.PARAM_RESULT(event.i32)).name))
                    stream_done.set()
                    stream_abort.set()
            elif event.type == ctl.EventType.STREAM_READY or event.type == ctl.EventType.STREAM_TRIGGERED:
                # These events are mainly useful when the 
                #STREAM_TRIGGER_MODE_EXTERNAL_ONCE trigger mode is used.
                # A STREAM_READY event is generated to indicate that the stream is ready
                # to be triggered by the external trigger. In this armed state the device
                # waits for the trigger to occur and then generates a
                # STREAM_TRIGGERED event.
                # This example uses the STREAM_TRIGGER_MODE_DIRECT trigger mode, thus we 
                # don't care about this events here.
                pass
            else:
                # The code should be prepared to handle unexpected events beside the
                # expected ones.
                print("MCS2 received event: {}".format(ctl.GetEventInfo(event)))

        except ctl.Error as e:
            if e.code == ctl.ErrorCode.CANCELED:
                # we use "INFINITE" timeout, so the function call will return only when
                # canceled by the "Cancel" function
...
try:
    ...
    # Sensor power mode: enabled (disable power save, which is not allowed with 
    # position streaming)
    for channel in range(NUMBER_OF_STREAMING_CHANNELS):
    # We commented out this command so the stage does not assume the first position
    #of the first frame to be the zero position
       # ctl.SetProperty_i64(d_handle, channel, ctl.Property.POSITION, 0)
        ctl.SetProperty_i32(d_handle, channel, ctl.Property.SENSOR_POWER_MODE, 
    ...
    # Set external sync rate to 100Hz (only active when using trigger mode 
    # STREAM_TRIGGER_MODE_EXTERNAL_SYNC)
    ...
    # (using STREAM_TRIGGER_MODE_DIRECT starts the stream as soon as enough frames were
    # sent to the device)
    s_handle = ctl.OpenStream(d_handle, ctl.StreamTriggerMode.DIRECT)
    # Send all frames in a loop
    # Note: the "AbortStream" function could be used to abort a running stream 
    # programmatically.
    ...
    print("Unexpected error: {}, {} in line: {}".format(ex, type(ex),
    (sys.exc_info()[-1].tb_lineno)))
    raise

finally:
    # Before closing the program the connection to the device must be closed by 
    # calling "Close".
    if d_handle != None:
        ctl.Close(d_handle)
    print("MCS2 close.")
